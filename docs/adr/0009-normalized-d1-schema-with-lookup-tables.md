...
`'sport'`, `'send'`, `'project'`, `'archived'`, `'checkout'` -- natural
keys are exactly what made #483's status renames, and #642/#658's
full `'lead'` -> `'sport'` discipline rename (migrations/0012 and
0013), plain `INSERT`/`UPDATE`/`DELETE` migrations rather than schema
changes). This keeps
