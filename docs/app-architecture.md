...
Seven separate build steps generate files under `public/logbook/` that
...
- **Client JS, one bundle per page**: `client:build`/`client:watch`
  ...
  since before #344) and its six #344/#302 siblings — `map:build`
  ...
  (`client/account-edit-main.js` → `account-edit-app.js`). All seven take
  ...
  follow this. `pnpm run deploy` builds all seven before `wrangler deploy`
  ...
  for them). `status-icons.js` moved into `client/` and into the bundle,
...
│                         /logbook's own; log-main.js/map-main.js/
│                         performance-main.js/account-main.js/
│                         account-edit-main.js each register it too (see
│                         each file's own comment) -- profile-main.js is
│                         the one newer page that doesn't
├── escape-html.js       Shared HTML-escaping helper — externalized into
│                         every one of the seven bundles above, not just
│                         app.js
...
### The newer split pages (epic #344, +#302)

`/logbook`'s own module graph above (`client/main.js` and everything it
composes) is unchanged by any of this. The six newer pages have their
...
│                           page) or client/main.js (its own equivalent
│                           inline logic stays as-is, per the
│                           parallel-migration policy)
├── fetch-json.js         loadResource(url, key) (#399) -- the fetch-with-
│                           cache-fallback helper duplicated across four
│                           of the six newer composition roots (log/map/
│                           performance/profile) -- account-main.js/
│                           account-edit-main.js have no logbook data of
│                           their own to fetch, so neither imports it. Not
│                           used by client/main.js, which has its own
│                           equivalent
...
(#206) — `client/main.js` is `/logbook`'s own entry point (one of seven
now, see "The newer split pages" above for the other six); pure-logic
...
2. **The six newer split pages** (`/log`, `/map`, `/performance`, the
   public `/:username` profile page, plus #302's `/account` and
   `/account/edit`) — each esbuild-bundled from its own composition root
   the same way `/logbook` is, sharing Web Components/a handful of
   pure-logic and view modules with `/logbook` and each other rather than
   duplicating them. See "The newer split pages" above for the full
   module inventory.
