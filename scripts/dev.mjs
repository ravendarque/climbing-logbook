/**
 * Starts the local dev server (wrangler + Tailwind watcher via the
 * `dev:raw` script), waits for it to be ready, seeds some test data, and
 * opens the app in a browser at the actual /logbook path -- all in one
...
const dev = spawn("pnpm", ["run", "dev:raw"], {
  ...
  // the readability a raw `pnpm run dev:raw` has for free.
  env: { ...process.env, FORCE_COLOR: "1" },
  // `dev:raw` runs concurrently, which runs wrangler and the Tailwind
  // watcher as further children -- killing just this direct child leaves
  // that whole subtree running. On POSIX, `detached: true` makes this
  // child the leader of its own process group, so
