...
test("uploads a valid CSV and shows the success summary", async ({ page }) => {
  ...
  await Promise.all([
    page.waitForResponse(res => res.url().includes("/-/api/admin/logbook/import") && res.request().method() === "POST"),
...
test("uploads a valid JSON export and shows the success summary, with the right Content-Type", async ({ page }) => {
  ...
  const [request] = await Promise.all([
    page.waitForRequest(req => req.url().includes("/-/api/admin/logbook/import") && req.method() === "POST"),
    page.locator("#import-submit-btn").click(),
  ]);
...
test("shows every row's error at once when the server rejects the file", async ({ page }) => {
  await mockApi(page);
  await page.route("**/-/api/admin/logbook/import", route =>
    route.fulfill({
      status: 400,
  ...
  await Promise.all([
    page.waitForResponse(res => res.url().includes("/-/api/admin/logbook/import")),
    page.locator("#import-submit-btn").click(),
  ]);
