...
// Every lookup ignores Vary: responses can carry `Vary: Origin`, and a
// pre-cached copy is stored under a plain URL with no Origin header, so a
// page's CORS request (module scripts, fonts) would otherwise never match
// it -- and fail offline. These are same-origin static files: the body
// doesn't vary by Origin.
...
// Whose shells to fetch: the owner of an open owner page. Registration
// only happens from owner pages (client/register-sw.js), so one is open
// unless it closed mid-install -- then the shells wait for the next
// "precache" message.
...
self.addEventListener("install", event => {
  ...
  self.skipWaiting();
  // #948 -- installed only once every item is cached. A failed attempt
  // keeps what it fetched, and the browser retries the install on the next
  // registration (every owner page load), which fetches only the rest.
...
// #948 -- every owner page asks once it's registered (client/register-sw.js).
// Tops up whatever this build's cache is missing: the shells an install
// had no username for, and everything after a logout cleared the caches.
// Nothing's fetched when the cache is complete.
