...
import { applyPendingQueue } from "./offline-queue.js";
    ...
    container.innerHTML = groups.map(([locationId, items]) => {
      ...
      const rows = sorted.map(e => {
        ...
        <tr class="border-b border-[color-mix(in_srgb,var(--color-border)_40%,transparent)] last:border-b-0 ${rowBg}">
          ...
          <td class="${TD_BASE} text-muted text-[.82rem] truncate">${escapeHtml(placeOf(e).area)}</td>
  ...
  // Grade range slider (#161): drag/keyboard/click-to-snap. Thumb identity
  // is fixed (min can never exceed max) rather than "whichever thumb is
  // closer" swapping mid-drag -- dragging one thumb into the other just
  // clamps at the shared index, which is also how the range collapses to
  // a single grade instead of needing a separate single/range mode.
  ...
  [[gradeThumbMin, "min"], [gradeThumbMax, "max"]].forEach(([thumb, which]) => {
    thumb.addEventListener("keydown", e => {
      ...
      else if (e.key === "End") { setGradeBound(which, activeGradeList().length - 1); e.preventDefault(); }
    });
  });
